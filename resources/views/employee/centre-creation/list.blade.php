                            ...
                            <div class="col-md-3">
                               ...
                               <div class="button-style-four btn-mg-b-10">
                                    @if(\App\Models\ProcessAppFlow::permission(2,1))
                                        <a class="btn btn-custon-four btn-default text-success add" href="{{ route('user.addCentreCreation') }}" title="Add"><i class="fa fa-plus"></i></a>
                                    @endif
                                    <a class="btn btn-custon-four btn-default text-success hardDltBtn" href="{{ route('user.trashedCentreCreation') }}" title="Restore"><i class="fa fa-trash"></i></a>
                                </div>
                               ...
                               @forelse($data as $result)
                                    <tr>
                                      ...
                                      <td class="">
                                        ...
                                        @endphp
                                        @if(($result->user_id == auth()->user()->id && \App\Models\ProcessAppFlow::permission(2,$result->status)) || \App\Models\ProcessAppFlow::permission(2,count(\App\Models\CentreCreation::status())))
                                            {!! Html::decode(link_to_route('user.editCentreCreation','<i class="fa fa-edit"></i>',[$result->slug],['class'=>'btn btn-custon-four btn-default text-info edit','title'=>'Edit'])) !!}

                                            {!! Html::decode(link_to_route('user.removeCentreCreation','<i class="fa fa-trash"></i>',[$result->slug],['class'=>'btn btn-custon-four btn-default text-danger remove','title'=>'Remove','onclick'=>'return removeData()'])) !!}
                                        @endif

                                        @if(\App\Models\ProcessAppFlow::permission(2,$cur_status) && $result->status < count(\App\Models\CentreCreation::status()))
                                            {!! Html::decode(link_to_route('user.centreCreationStatusView','<i class="fa fa-check"></i>',[$result->slug],['class'=>'btn btn-custon-four btn-default text-success Approve','title'=>'Approve'])) !!}
                                        @endif
                                      </td>
                                    </tr>
                                   @empty
