                            ...
                            <div class="col-md-3">
                               ...
                               <div class="button-style-four btn-mg-b-10">
                                    @if(\App\Models\ProcessAppFlow::permission(1,1))
                                        <a class="btn btn-custon-four btn-default text-success add" href="{{ route('user.addProject') }}" title="Add"><i class="fa fa-plus"></i></a>
                                    @endif
                                    <a class="btn btn-custon-four btn-default text-success hardDltBtn" href="{{ route('user.trashedProject') }}" title="Restore"><i class="fa fa-trash"></i></a>
                                </div>
                               ...
                               @forelse($data as $result)
                                    <tr>
                                      ...
                                      <td class="">
                                        ...
                                        @endphp
                                        @if(($result->user_id == auth()->user()->id && \App\Models\ProcessAppFlow::permission(1,$result->status)) || \App\Models\ProcessAppFlow::permission(1,count(\App\Models\Project::status())))
                                            {!! Html::decode(link_to_route('user.editProject','<i class="fa fa-edit"></i>',[$result->slug],['class'=>'btn btn-custon-four btn-default text-info edit','title'=>'Edit'])) !!}

                                            {!! Html::decode(link_to_route('user.removeProject','<i class="fa fa-trash"></i>',[$result->slug],['class'=>'btn btn-custon-four btn-default text-danger remove','title'=>'Remove','onclick'=>'return removeData()'])) !!}
                                        @endif

                                        @if(\App\Models\ProcessAppFlow::permission(2,$cur_status) && $result->status < count(\App\Models\Project::status()))
                                            {!! Html::decode(link_to_route('user.projectStatusView','<i class="fa fa-check"></i>',[$result->slug],['class'=>'btn btn-custon-four btn-default text-success Approve','title'=>'Approve'])) !!}
                                        @endif
                                      </td>
                                    </tr>
                                   @empty
