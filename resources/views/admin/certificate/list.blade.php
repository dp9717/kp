                                        ...
                                        <td class="">
                                          <button type="button" class="btn btn-custon-four btn-default text-success view" data-slug="{{ $result->slug }}" title="View">
                                            <i class="fa fa-eye"></i>
                                              ...
                                              {!! Html::decode(link_to_route('admin.editCertificate','<i class="fa fa-edit"></i>',[ $result->slug ],['class'=>'btn btn-custon-four btn-default text-info edit','title'=>'Edit'])) !!}
                                          

                                          {!! Html::decode(link_to_route('admin.downloadCertificate','<i class="fa fa-file-pdf-o"></i>',[ 'batch' => \App\Models\Batch::slug($result->batch_id), 'student' => $s_val->slug, 'slug' => $result->slug ],['class'=>'btn btn-custon-four btn-default text-info download','title'=>'Edit'])) !!}
                                        </td>
                                      </tr>
                                    @endforeach
                                  @endif
...
<script type="text/javascript">
  $(document).ready(function () {
    ...
    $(document).on('click', '.certiBtn', function() {
      ...
      var url="{{ route('admin.bulkCertificate') }}";
      ...
      if (stArray.length > 0) {
        ...
        $.ajax({
          url: url,
          type: "post",
          data: { 'student': student, 'batch': batch, 'certificate': certificate , _token: '{{csrf_token()}}',},
          success: function(response){
