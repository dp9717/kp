...
class CentreCreationExport extends DefaultValueBinder implements FromCollection, WithMapping, WithHeadings, WithEvents, WithCustomValueBinder
{
                    ...
                    if($key==0){
                        if ($row['id']!='') {
                            ...
                            $output[] = [
                                ...
                                $row['email'] ?? '',
                                $row['contact'] ?? '',
                                \App\Models\User::userAry($row['centre_head_id'])->name ?? '',
                                $add,
