...
class AssesmentExport extends DefaultValueBinder implements FromCollection, WithMapping, WithHeadings, ShouldAutoSize, WithEvents, WithCustomValueBinder
{
                            ...
                            if (isset($row['att_student']) && count($row['att_student'])) {
                                foreach ($row['att_student'] as $s_key => $s_value) {
                                    $output[] = [
                                        ...
                                        Assesment::grade($s_value['grade'] ?? '-1'),
