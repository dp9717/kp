...
class AdminVendorController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('admin.vendor.list',compact('data','search','page','total','account_status'));
    ...
    public function edit($slug)
    {
        ...
        if($data){
            ...
            return view('admin.vendor.edit',['state'=>$state,'city'=>$city,'taluk'=>$taluk,'pincode'=>$pincode,'pincode'=>$pincode,'data'=>$data]);
                ...
                $data->contact_to = Helper::date($request->contact_to,'Y-m-d');
                 if($data->save()){
                    ...
                    if ($data->status == count(Vendor::status())) {
                        return to_route('admin.vendorApprovedList')->with('success', 'Updated successfully !');
                    }
                    return to_route('admin.vendors')->with('success', 'Updated successfully !');
                 }else{
                     return back()->with('failed', 'Wrong access or try again.');
                 } 
        }else{
           return redirect()->route('admin.vendors')->with('failed', 'Wrong access or try again.');
        }
    }
    ...
    public function remove($slug)
    {
        ...
        $data=Vendor::where('slug',$slug)->first();
        if ($data && $data->status < count(\App\Models\Vendor::status())) {
                if($data->delete()){
    ...
    public function trashedData(Request $request)
    {
        ...
        $data=$data->paginate($this->paginate);
        $page = ($data->perPage()*($data->currentPage() -1 ));
        return view('admin.vendor.trashedList',compact('data','search','page','total','account_status'));
    ...
    public function statusView($slug)
    {
        ...
        if($data){
            ...
            if($data->status < count(\App\Models\Vendor::status())){
                $status = Vendor::statusApprovalArray($data->status);
                $users = Vendor::authUserByStatus($cur_status);
                return view('admin.vendor.statusView',['status'=>$status,'data'=>$data,'users'=>$users]);
            }else{
        ...
        if($data){
            ...
            if($data->status < count(\App\Models\Vendor::status())){
                ...
                if ($request->status==2) {
                 ...
                 if($data->save()){
                    return to_route('admin.vendors')->with('success', 'Request status updated successfully !');
                 }else{
                     return back()->with('failed', 'Wrong access or try again.');
                 }
            }else{
    ...
    public function approvedList(Request $request)
    {
        ...
        $data=$data->paginate($this->paginate);
        $page = ($data->perPage()*($data->currentPage() -1 ));
        return view('admin.vendor.approved',compact('data','search','page','total','account_status'));
