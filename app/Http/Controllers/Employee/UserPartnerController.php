...
class UserPartnerController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('employee.partner.list',compact('data','search','page','total','account_status'));
    ...
    public function add($value='')
    {
        ...
        return view('employee.partner.add',['state'=>$state,'city'=>$city,'taluk'=>$taluk,'pincode'=>$pincode,'pincode'=>$pincode]);
    }
    public function edit($slug)
    {
        ...
        if($data){
            if (($data->user_id==auth()->user()->id && $data->status < count(\App\Models\Partner::status())) || \App\Models\ProcessAppFlow::permission(5,count(\App\Models\Partner::status()))) {
                $data->status = 1;
                $state = State::statePluck(); 
                $city = City::cityPluck($data->state_id ?? 0);
                $taluk = Taluk::talukPluck($data->city_id ?? 0);
                $pincode =Pincode::pincodePluck($data->taluk_id ?? 0);
                return view('employee.partner.edit',['state'=>$state,'city'=>$city,'taluk'=>$taluk,'pincode'=>$pincode,'pincode'=>$pincode,'data'=>$data]);
            }else{
    ...
    public function insert(PartnerRequest $request)
    {
        if(\App\Models\ProcessAppFlow::permission(5,1)){
            $path = $this->path;
            $upload_file =  '';
            ...
            if(\App\Models\ProcessAppFlow::permission(5,3)){
                $data->status = 3;
            ...
            if ($request->director_name) {
             ...
             if($data->save()){
                $data->slug=Helper::ProcessUniqueCode($data->id,5);
                $data->save();
                if ($request->upload_file) {
                ...
                return to_route('user.partners')->with('success', 'Saved successfully !');
             }else{
                 return back()->with('failed', 'Wrong access or try again.');
             }
        }else{
    ...
    public function update(PartnerRequest $request,$slug=null)
    {
        ...
        if($data){
            if (($data->user_id==auth()->user()->id && $data->status < count(\App\Models\Partner::status())) || \App\Models\ProcessAppFlow::permission(5,count(\App\Models\Partner::status()))) {
                ...
                $data->pincode_id = $p_data->id;
                $data->full_address = json_encode(Pincode::fullAddress($request->pincode,Helper::removetag($request->address)));
                if(\App\Models\ProcessAppFlow::permission(5,count(\App\Models\Partner::status()))){
                    $data->status = 3;
                    $data->second_level_id = Auth()->user()->id;
                    $data->second_level_ary = json_encode(User::userAry(Auth()->user()->id));
                }
                if ($data->status==2 && (\App\Models\ProcessAppFlow::permission(5,count(\App\Models\Partner::status()))==0)) {
                    $data->status = 1;
                }
                if ($request->poc_name) {
                    $itemDetail=[];
                ...
                if ($request->director_name) {
                 ...
                 if($data->save()){
                    ...
                    if ($data->status == count(Partner::status())) {
                        return to_route('user.partnerApprovedList')->with('success', 'Updated successfully !');
                    }
                    return to_route('user.partners')->with('success', 'Updated successfully !');
                 }else{
                     return back()->with('failed', 'Wrong access or try again.');
                 } 
           ...
           return redirect()->route('user.partners')->with('failed', 'Wrong access or try again.');
        }
    }
    ...
    public function trashedData(Request $request)
    {
        ...
        $data=$data->paginate($this->paginate);
        $page = ($data->perPage()*($data->currentPage() -1 ));
        return view('employee.partner.trashedList',compact('data','search','page','total','account_status'));
    ...
    public function statusView($slug)
    {
        ...
        if($data){
            ...
            if(\App\Models\ProcessAppFlow::permission(5,$cur_status) && $data->status < count(\App\Models\Partner::status())){
                $status = Partner::statusApprovalArray($data->status);
                return view('employee.partner.statusView',['status'=>$status,'data'=>$data]);
            }else{
        ...
        if($data){
            ...
            if(\App\Models\ProcessAppFlow::permission(5,$cur_status) && $data->status < count(\App\Models\Partner::status())){
                ...
                if ($request->status==2) {
                 ...
                 if($data->save()){
                    return to_route('user.partners')->with('success', 'Request status updated successfully !');
                 }else{
                     return back()->with('failed', 'Wrong access or try again.');
                 }
            }else{
    ...
    public function approvedList(Request $request)
    {
        ...
        $data=$data->paginate($this->paginate);
        $page = ($data->perPage()*($data->currentPage() -1 ));
        return view('employee.partner.approved',compact('data','search','page','total','account_status'));
