...
return new class extends Migration
{
    ...
    {
        Schema::create('centre_creations', function (Blueprint $table) {
            $table->id();
            $table->timestamps();
            $table->softDeletes();
            ...
            // centre head user
            $table->unsignedBigInteger('centre_head_id');
            $table->foreign('centre_head_id')->references('id')->on('users');
            $table->text('centre_head_ary')->nullable();
            // created user
    ...
    {
        Schema::dropIfExists('centre_creations');
    }
};
